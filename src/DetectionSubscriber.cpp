...
public:
      ...
      : Node("detection_subscriber")
  {
    ...
    declare_parameter("subscribers.darknet_ros_mno.topic", std::string("/darknet_ros_mno"));
    declare_parameter("subscribers.way_point.topic", std::string("/current_waypoint_index"));
    declare_parameter("publishers.is_runnable.topic", std::string("/is_runnable"));
  }
  ...
  void init(){
    ...
    std::string isRunnableTopicName;
    get_parameter("detect_way_points", detectWayPoints);
    get_parameter("subscribers.darknet_ros_mno.topic", mnoDarknetTopicName);
    get_parameter("subscribers.way_point.topic", wayPointTopicName);
    get_parameter("publishers.is_runnable.topic", isRunnableTopicName);

    subscription_darknet_ros = this->create_subscription<darknet_ros_msgs::msg::Mno>(
        mnoDarknetTopicName, 10, std::bind(&DetectionSubscriber::mnoDarknetCallback, this, _1));
    subscription_way = this->create_subscription<std_msgs::msg::Int16>(
        wayPointTopicName, 10, std::bind(&DetectionSubscriber::wayPointCallback, this, _1));
    publisher_ = this->create_publisher<std_msgs::msg::Bool>(isRunnableTopicName, 10);
  }

private:
  void mnoDarknetCallback(const darknet_ros_msgs::msg::Mno::SharedPtr msg) const
  {
  ...
  void wayPointCallback(const std_msgs::msg::Int16::SharedPtr msg) const
  {
    ...
    RCLCPP_INFO(this->get_logger(), "way point = %d", wayPoint);
  ...
  rclcpp::Subscription<darknet_ros_msgs::msg::Mno>::SharedPtr subscription_darknet_ros;
...
int main(int argc, char *argv[])
{
  ...
  auto mnoPedeDetector = std::make_shared<DetectionSubscriber>();
